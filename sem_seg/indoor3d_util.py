...
g_classes = [x.rstrip() for x in open(os.path.join(BASE_DIR, 'meta/class_namesbmy.txt'))]
g_class2label = {cls: i for i,cls in enumerate(g_classes)}
g_class2color = {'body':   [0,255,0],
         'head':           [0,0,255],
         'leftarm':        [0,255,255],
         'leftleg':        [255,255,0],
        'rightarm':        [255,0,255],
         'rightleg':       [100,100,255],
          'lefthand':        [200,200,100],
         'righthand':       [170,120,200] }
...
def collect_point_label(anno_path, out_filename, file_format='txt'):
  ...
  for f in glob.glob(os.path.join(anno_path, '*.txt')):
    print(f)
    cls = os.path.basename(f).split('_')[0]   #从给定文件的路径中，获取最后一个/后的内容，split是以_拆开数组，[0]是数组拆开后第一个元素
    ...
    points_list.append(np.concatenate([points, labels], 1)) # Nx7     #这个是xyzrgb的
...
def room2blocks(data, label, num_point, block_size=1.0, stride=1.0,
  ...
  for idx in range(len(xbeg_list)):
     xbeg = xbeg_list[idx]
     #print(xbeg)                                                 #xbeg从0开始到320，每0.5一加
     ybeg = ybeg_list[idx]
     xcond = (data[:,0]<=xbeg+block_size) & (data[:,0]>=xbeg)     #data[:,0]表示第1列所有数据
     #print(data[:,0])                                            #316到278
     #print(xbeg+block_size)
     #print(xbeg)
     #print(xcond)                                                #xcond全是false
     ...
     if np.sum(cond) < 100: # discard block if there are less than 100 pts.        这里出了问题，cond全是0
       continue
     ...
     #print(np.concatenate(block_data_list, 0),np.concatenate(block_label_list, 0))
