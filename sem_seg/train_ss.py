...
parser.add_argument('--gpu', type=int, default=0, help='GPU to use [default: GPU 2]')
parser.add_argument('--model', default='sseg', help='Model name [default: pan3_sseg]')
...
parser.add_argument('--test_room_data_filelist', default='./meta/area5_data_label.txt',
...
print(len(label_batch_list))
data_batches = np.concatenate(data_batch_list, 0)
label_batches = np.concatenate(label_batch_list, 0)
del data_batch_list,label_batch_list
...
def eval_one_epoch(sess, ops, test_writer):
	...
	#ZeroDivisionError: float division by zero +float("1e-8)
	current_all_acc = (sum(true_positive_classes)/(float(sum(positive_classes)+float("1e-8"))))
	log_string('overall accuracy: %f' % current_all_acc)

	class_list = []
	for i in range(8):
		acc_class = true_positive_classes[i]/(float(gt_classes[i]+float("1e-8")))

		class_list.append(acc_class)

	current_cls_acc = (sum(class_list)/8.0)
	log_string('avg class accuracy: %f' % current_cls_acc)
	log_string ('IoU: ')
	iou_list = []
	for i in range(8):
		iou = true_positive_classes[i]/(float(gt_classes[i]+positive_classes[i]-true_positive_classes[i]+float("1e-8")))
		log_string('%f' % iou)
		iou_list.append(iou)

	current_mean_iou = (sum(iou_list)/8.0)
